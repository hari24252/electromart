...
export const adminAuthRepository = {
  ...
  updatePassword: (id: string, passwordHash: string) => Admin.findByIdAndUpdate(id, { passwordHash, $unset: { refreshSessionId: 1 }, $inc: { authVersion: 1 } }, { returnDocument: 'after' }),
  updateLastLogin: (id: string) => Admin.findByIdAndUpdate(id, { lastLoginAt: new Date() }),
  invalidateSessions: (id: string) => Admin.findByIdAndUpdate(id, { $unset: { refreshSessionId: 1 }, $inc: { authVersion: 1 } }, { returnDocument: 'after' }),
