...
export const adminAuthController = {
  login: asyncHandler(async (req, res) => { const tokens = await adminAuthService.login(req.body.email, req.body.password); putRefreshCookie(res, tokens.refreshToken); return success(res, { accessToken: tokens.accessToken }, 'Administrator signed in'); }),
  ...
  refreshToken: asyncHandler(async (req, res) => { const tokens = await adminAuthService.refresh(req.cookies[refreshCookieName('admin')] as string); putRefreshCookie(res, tokens.refreshToken); return success(res, { accessToken: tokens.accessToken }, 'Token refreshed'); }),
